...
use super::GraphifyGraph;

/// Build a graph from a single extraction result.
pub fn build_from_extraction(extraction: &ExtractionResult) -> GraphifyGraph {
    let mut graph = GraphifyGraph::new();
...
pub fn build_merged(extractions: &[ExtractionResult]) -> GraphifyGraph {
    let mut graph = GraphifyGraph::new();
