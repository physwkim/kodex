...
pub fn download_audio(url: &str, output_dir: &Path) -> crate::error::Result<PathBuf> {
    ...
    entries
        ...
        .map(|e| e.path())
        .ok_or_else(|| {
            crate::error::GraphifyError::Other("No audio file found after download".to_string())
        })
}
...
pub fn build_whisper_prompt(god_node_labels: &[String]) -> String {
    ...
    if let Ok(custom) = std::env::var("GRAPHIFY_WHISPER_PROMPT") {
        if !custom.is_empty() {
            return custom;
        }
    }
...
pub fn transcribe(
    ...
    let model_name = std::env::var("GRAPHIFY_WHISPER_MODEL").unwrap_or_else(|_| "base".to_string());
    let model_path = resolve_model_path(&model_name)?;

    // Load model
    let ctx = WhisperContext::new_with_params(&model_path, WhisperContextParameters::default())
        .map_err(|e| crate::error::GraphifyError::Other(format!("Failed to load Whisper model: {e}")))?;
    ...
    let mut state = ctx.create_state()
        .map_err(|e| crate::error::GraphifyError::Other(format!("Failed to create state: {e}")))?;

    state.full(params, &samples)
        .map_err(|e| crate::error::GraphifyError::Other(format!("Transcription failed: {e}")))?;
...
fn load_audio_samples(path: &Path) -> crate::error::Result<Vec<f32>> {
    ...
    let wav_path = if ext == "wav" {
        ...
        if !status.success() {
            return Err(crate::error::GraphifyError::Other(
    ...
    let reader = hound::WavReader::open(&wav_path)
        .map_err(|e| crate::error::GraphifyError::Other(format!("Failed to read WAV: {e}")))?;
...
fn resolve_model_path(model_name: &str) -> crate::error::Result<String> {
    ...
    if let Ok(path) = std::env::var("GRAPHIFY_WHISPER_MODEL_PATH") {
    ...
    Err(crate::error::GraphifyError::Other(format!(
        "Whisper model '{filename}' not found.\n\
         Download from: https://huggingface.co/ggerganov/whisper.cpp/tree/main\n\
         Place in ~/.cache/whisper/ or set GRAPHIFY_WHISPER_MODEL_PATH env var."
    )))
}
